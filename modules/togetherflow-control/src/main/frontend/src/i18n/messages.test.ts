...
import { controlEn, controlMessages } from "./messages";
...
const merged = { ...commonEn, ...controlEn };

describe("Control message catalogue", () => {
  ...
  it("has no entries nothing references", () => {
    ...
    expect(unusedKeys(usage, controlEn)).toEqual([]);
  });
  ...
  it("keeps every locale in step with the source locale", () => {
    ...
    expect(localeGaps(mergeCatalogues(commonMessages, controlMessages))).toEqual({});
  });
});
