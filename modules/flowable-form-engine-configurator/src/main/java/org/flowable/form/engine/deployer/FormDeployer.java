...
public class FormDeployer implements EngineDeployer {
        ...
        for (String resourceName : resources.keySet()) {
            if (resourceName.endsWith(".form")) {
                ...
                if (formDeploymentBuilder == null) {
                    FormRepositoryService formRepositoryService = formRepositoryService();
    ...
    public void undeploy(EngineDeployment parentDeployment, boolean cascade) {
        FormRepositoryService repositoryService = formRepositoryService();
        List<FormDeployment> deployments = repositoryService
                .createDeploymentQuery()
                .parentDeploymentId(parentDeployment.getId())
                .list();

        for (FormDeployment deployment : deployments) {
            repositoryService.deleteDeployment(deployment.getId(), cascade);
        }
    }

    /** The form engine registered on the current command context — whichever engine is running the deployment. */
    protected FormRepositoryService formRepositoryService() {
        FormEngineConfiguration formEngineConfiguration = CommandContextUtil.getFormEngineConfiguration(Context.getCommandContext());
        return formEngineConfiguration != null ? formEngineConfiguration.getFormRepositoryService() : null;
    }
}
