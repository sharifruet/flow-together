...
import { Button, validateVariable, type EditableVariable, type EditableVariableType } from "@togetherflow/common";
...
export function VariableEditor({
  ...
  return (
    <div className="tf-variables">
      {variables.length === 0 ? (
        <p className="tf-muted tf-variables__empty">No variables.</p>
      ) : (
              ...
              <li className="tf-variables__row" key={index}>
                ...
                <div className="tf-variables__value">
                  <label className="tf-visually-hidden" htmlFor={`var-value-${index}`}>
                    Value for {variable.name || "new variable"}
                  </label>
                  {variable.type === "json" ? (
                    <textarea
                      id={`var-value-${index}`}
