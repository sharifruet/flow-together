...
| A case opens with its elements newly arranged | The `.cmmn` had no CMMNDI, so Design laid it out | Not a fault. Diagram interchange is optional and hand-written case files routinely omit it; the elements used to land on one coordinate, which looked like an empty case. The layout is a starting point and is saved like any other edit, so a case goes through it once |
