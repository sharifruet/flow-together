...
import { identityEn, identityMessages } from "./messages";
...
const merged = { ...commonEn, ...identityEn };

describe("Identity message catalogue", () => {
  ...
  it("has no entries nothing references", () => {
    ...
    expect(unusedKeys(usage, identityEn)).toEqual([]);
  });
  ...
  it("keeps every locale in step with the source locale", () => {
    ...
    expect(localeGaps(mergeCatalogues(commonMessages, identityMessages))).toEqual({});
  });
});
