/**
 * TogetherFlow Shell for the Identity app (REQUIREMENTS.md §7.5).
 *
...
const FUTURE_APPS = ["Control", "Design"];
    ...
    <div className="tf-shell">
      ...
      <header className="tf-shell__header">
        <div className="tf-shell__brand">
          <Brand size={26} />
          <span className="tf-shell__app-name">Identity</span>
        </div>

        <nav className="tf-shell__nav" aria-label="Identity sections">
          {(["users", "groups", "privileges"] as IdentityView[]).map((section) => (
            <button
              key={section}
              type="button"
