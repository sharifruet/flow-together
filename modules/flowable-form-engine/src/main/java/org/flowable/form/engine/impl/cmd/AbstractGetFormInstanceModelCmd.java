...
public class AbstractGetFormInstanceModelCmd implements Command<FormInstanceInfo>, Serializable {
    ...
    public FormInstanceInfo execute(CommandContext commandContext) {
        ...
        if (formDefinitionId == null && formDefinitionCacheEntry != null && formDefinitionCacheEntry.getFormDefinitionEntity() != null) {
            // Resolved by key: the instance lookup must be scoped to this definition, or a case/process with
            // several forms would hand back whichever instance was submitted first.
            formDefinitionId = formDefinitionCacheEntry.getFormDefinitionEntity().getId();
        }
    ...
    protected void initializeValuesForScope(String formDefinitionKey, String parentDeploymentId, String formDefinitionId, String tenantId,
        ...
        initializeValuesForScope(formDefinitionKey, parentDeploymentId, formDefinitionId, tenantId, null, scopeId, scopeType, variables, fallbackToDefaultTenant);
    }

    protected void initializeValuesForScope(String formDefinitionKey, String parentDeploymentId, String formDefinitionId, String tenantId,
            String taskId, String scopeId, String scopeType, Map<String, Object> variables, boolean fallbackToDefaultTenant) {
            ...
            for (FormField field : allFields) {
                if (field instanceof OptionFormField) {
                    ...
                    if (variableValue != null) {
                        ...
                        if (FormFieldTypes.DATE.equals(field.getType())) {
                            field.setValue(FormValueUtil.toDateFieldValue(variableValue));
                        } else {
                            field.setValue(variableValue);
                        }
                    }
        ...
        try {
            ...
            if (submittedNode.get("values") != null) {
                ...
                for (Map.Entry<String, JsonNode> entry : valuesNode.properties()) {
                    formInstanceFieldMap.put(entry.getKey(), entry.getValue());
                }
            }

            if (submittedNode.get("flowable_form_outcome") != null) {
                JsonNode outcomeNode = submittedNode.get("flowable_form_outcome");
                if (!outcomeNode.isNull() && StringUtils.isNotEmpty(outcomeNode.asString())) {
                    formInstanceModel.setSelectedOutcome(outcomeNode.asString());
                }
            }
    ...
    public void fillVariablesWithFormValues(Map<String, JsonNode> submittedFormFieldMap, List<FormField> allFields) {
        for (FormField field : allFields) {
            ...
            String fieldValue = fieldValueNode.asString();

            if (FormFieldTypes.DATE.equals(fieldType)) {
                try {
                    if (StringUtils.isNotEmpty(fieldValue)) {
                        variables.put(field.getId(), FormValueUtil.toDateFieldValue(fieldValue));
                    }
                } catch (Exception e) {
