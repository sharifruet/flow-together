...
import { workEn, workMessages } from "./messages";
...
const merged = { ...commonEn, ...workEn };

describe("Work message catalogue", () => {
  ...
  it("has no entries nothing references", () => {
    ...
    expect(unusedKeys(usage, workEn)).toEqual([]);
  });
  ...
  it("keeps every locale in step with the source locale", () => {
    ...
    expect(localeGaps(mergeCatalogues(commonMessages, workMessages))).toEqual({});
  });
});
